...
async function run() {
  try {
    ...
    const ordersWithObj = await Order.find({ user: new mongoose.Types.ObjectId(userId) }).lean();
    console.log("Find by Mongoose ObjectId results count:", ordersWithObj.length);
  } catch (err) {
    console.error(err);
  } finally {
